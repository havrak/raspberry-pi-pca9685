...
PCA9685::PCA9685(uint8_t nAddress)
		: m_nAddress(nAddress)
{
	I2cSetup();
	std::cout << "I2cSetup fd: " << fd << "\n";

	int settings = wiringPiI2CReadReg8(fd, PCA9685_REG_MODE1) & 0x7F;
	int autoInc = settings | 0x20;

	wiringPiI2CWriteReg8(fd, PCA9685_REG_MODE1, autoInc);
...
bool PCA9685::I2cSetup()
{
	// FUNC_PREFIX(i2c_set_address(m_nAddress));
	fd = wiringPiI2CSetup(m_nAddress);
	return fd >= 0;
...
{
	wiringPiI2CWriteReg16(fd, reg	 , data);
	wiringPiI2CWriteReg16(fd, reg + 2, data2);
